...
This module lives at `bindings/go/` inside the [smartmontools-sdk](https://github.com/dianlight/smartmontools-sdk)
monorepo and is tagged independently as `bindings/go/vX.Y.Z`. See
[docs/migration/smartmontools-go-to-bindings-go.md](../../docs/migration/smartmontools-go-to-bindings-go.md)
if you are migrating from the former standalone `smartmontools-go` repository.

![CI](https://github.com/dianlight/smartmontools-sdk/actions/workflows/ci-bindings-go.yml/badge.svg)
[![Coverage Status](https://codecov.io/github/dianlight/smartmontools-sdk/graph/badge.svg?flag=bindings-go)](https://codecov.io/github/dianlight/smartmontools-sdk)
![Latest bindings/go tag](https://img.shields.io/github/v/tag/dianlight/smartmontools-sdk?filter=bindings%2Fgo%2F*&label=bindings%2Fgo)
...
it loads a smartmon wrapper shared library (`libsmartmon_go.so` / `.dylib`) at runtime. The
wrapper is built in-tree from the monorepo root — there is no pre-built or downloadable
artifact:

```bash
# 1. Build the native core (native/)
...
# 2. Build the C ABI wrapper against it
scripts/build-wrapper.sh
```

This compiles the thin C++ wrapper (`native/capi/smartmon_c_api.cpp`) against `libsmartmon.a`
and produces `packaging/artifacts/lib/libsmartmon_go.{so,dylib}`.
...
lib, err := libbackend.New(
    libbackend.WithLibraryPath("packaging/artifacts/lib/libsmartmon_go.dylib"), // or .so on Linux
)
if err != nil {
...
```bash
# macOS — after building the wrapper with scripts/build-wrapper.sh
SMARTMON_LIB_PATH=packaging/artifacts/lib/libsmartmon_go.dylib go run .

# Linux — after building the wrapper with scripts/build-wrapper.sh
...
# LibBackend (after building the wrapper with scripts/build-wrapper.sh)
SMARTMON_LIB_PATH=../../../../packaging/artifacts/lib/libsmartmon_go.dylib \
  cd examples/lib && go run .

# CompareBackend: ExecBackend master + LibBackend shadow
SMARTMON_LIB_PATH=../../../../packaging/artifacts/lib/libsmartmon_go.dylib \
...
Loads the smartmon wrapper shared library (`libsmartmon_go.so` /
`libsmartmon_go.dylib`), built in-tree from `native/` via `scripts/build-wrapper.sh`, at
runtime using [ebitengine/purego](https://github.com/ebitengine/purego) — **no CGO required**.
...
This module is licensed under the GNU General Public License v2.0 or later - see the
[COPYING](../../COPYING) file at the repository root for details.
...
This module uses [`mise`](https://mise.jdx.dev/) for task running. CI runs via the monorepo's
`ci-bindings-go.yml` workflow, triggered on `push` and `pull_request` to `main` when files under
`bindings/go/**`, `native/capi/**`, or `native/drivedb/**` change.
...
| `mise run release [major\|minor\|patch]` | Create and push a `bindings/go/vX.Y.Z` tag — stable on `main`, prerelease on a branch with an open PR (requires `gh`) |
