...
// library via purego (no CGO required). Build the native core and the wrapper
...
//
//go:generate ../../../../scripts/build-wrapper.sh
package lib
...
// Required C ABI version. major must match exactly (a differing major means
// removed/renamed/resignatured symbols); minor may be greater than or equal to
// this (minor changes are additive-only, never removing or resignaturing a
// symbol the binding already relies on).
const (
	abiMajorRequired = 1
	abiMinorRequired = 0
)
...
// purego at runtime. No CGO is required. Build it in-tree from the repository
// root with scripts/build-wrapper.sh, after building native/ (see
// backends/lib/shared.go for the full sequence).
...
func New(opts ...Option) (*LibBackend, error) {
	...
	if err := checkABIVersion(funcs); err != nil {
		_ = purego.Dlclose(handle)
		return nil, fmt.Errorf("%s: %w", b.libPath, err)
	}
	b.funcs = funcs
	b.libHandle = handle
...
func resolveLibPath() (string, error) {
	...
	return "", errors.New("smartmon wrapper library (libsmartmon_go.{so,dylib}) not found; build it in-tree with scripts/build-wrapper.sh (after building native/, see backends/lib/shared.go), then set SMARTMON_LIB_PATH or copy to a standard library directory")
}
...
// checkABIVersion enforces the three-tier semver contract encoded in
// smartmon_abi_version(): major must match exactly, minor must be >= what this
// binding was built against. purego.RegisterLibFunc does not validate function
// signatures, so a mismatched major version binds successfully and would
// otherwise cause undefined behaviour rather than a clean, detectable failure.
func checkABIVersion(f *libFuncs) error {
	...
	if major != abiMajorRequired || minor < abiMinorRequired {
		return fmt.Errorf(
			"incompatible smartmon C ABI version %d.%d.%d (this binding requires major=%d, minor>=%d); "+
				"rebuild the wrapper with scripts/build-wrapper.sh",
			major, minor, patch, abiMajorRequired, abiMinorRequired)
	}
	return nil
}
...
func registerFuncs(lib uintptr) (*libFuncs, error) {
	...
	for _, b := range []struct {
		...
		{&f.abiVersion, "smartmon_abi_version"},
