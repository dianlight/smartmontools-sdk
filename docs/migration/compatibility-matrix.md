...
| — | `v8.0.0` **(retracted)** | `bindings/go/v8.0.0` **(retracted)** | 1.0 | First monorepo release, under the shared-version cascade (see [../development/release-process.md](../development/release-process.md)). Module path change, relicense to GPL-2.0-or-later, `smartmon_abi_version()` introduced, wrapper now built in-tree from `native/`. Supersedes the `bindings/go/v0.5.0` heading once carried in `bindings/go/CHANGELOG.md` — that version was never actually tagged (`git tag -l 'bindings/go/*'` was empty), so there is no `v0.5.0` release to migrate from. **Published in error**: cut as a final release while upstream smartmontools 8.0 was (and still is) unreleased. Retracted via `bindings/go/go.mod`'s `retract` block — see the tombstone row below and [../development/release-process.md](../development/release-process.md#the-v800--v801-tombstone). Never delete this tag or release: it is permanently in the Go checksum database regardless, and the retraction can only work by superseding it, not by hiding it. |
| — | `v8.0.1` **(tombstone)** | `bindings/go/v8.0.1` **(tombstone)** | 1.0 | Stable, no-code-change release whose only purpose is to self-retract alongside `v8.0.0` above, draining Go's `@latest` query past both so it falls through to the newest `-pre.N` prerelease below. Do not delete; do not treat as a real release to build against. |
| — | `v8.0.2-pre.<revs>` | `bindings/go/v8.0.2-pre.<revs>` | 1.0 | First release under the corrected scheme: the core is genuinely still a prerelease (upstream smartmontools 8.0 is not yet released), so the tag says so. `<revs>` is a commit count, not a fixed number — it advances with every prerelease cut from `scripts/prerelease-state.sh --revs`. Requires an explicit `@v8.0.2-pre.<revs>` pin to install (see [../development/release-process.md](../development/release-process.md#consuming-a-prerelease)); graduates to a plain `v8.0.2` (or a later `v8.0.N`) the first time upstream sets `native/configure.ac`'s release date for this generation. |
